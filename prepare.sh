...
for base in $BASES; do
    curl -fsSL "$(curl -fsS -H "X-Ubuntu-Series: 16" "https://api.snapcraft.io/api/v1/snaps/details/$base" | jq ".download_url" -r)" --output "$base.snap"
    mkdir -p "/snap/$base"
...
# place.
curl -fsSL "$(curl -fsS -H "X-Ubuntu-Series: 16" "https://api.snapcraft.io/api/v1/snaps/details/snapcraft?channel=$RISK" | jq ".download_url" -r)" --output snapcraft.snap
